    ...
    <div v-if="libraryData">
        ...
        <div class="flex-wrapper">
          <div class="books-grid">
            <div v-for="item in popularBooksDetail" :key="item.detailData.isbn13">
              <img
                :src="item.detailData.bookImageURL"
                height="237"
                width="206"
                alt="book-img"
                @click="handleBookClick(item.detailData.isbn13)"
                class="book-image"
          ...
          <div
            ...
            :key="item.detailData.isbn13"
            class="popular-book-info"
          >
      ...
      <div class="popular-books">
        <div class="title-with-tab">
          <h2>도서관 연령별 인기 도서</h2>
          <div :class="{tab : true, 'tab-active': currentAgeFilter === 'age20Books'}" @click="handleTabClick('age20Books')">성인</div>
          <div :class="{tab : true, 'tab-active': currentAgeFilter === 'age14Books'}" @click="handleTabClick('age14Books')">청소년</div>
          <div :class="{tab : true, 'tab-active': currentAgeFilter === 'age8Books'}" @click="handleTabClick('age8Books')">영유아</div>
        </div>
        <div class="books-flex">
          <HomeBookItem
            v-for="book in popularBooks[currentAgeFilter]"
      ...
      <KakaoMap class="map" :width="1246" :height="400" :lat="Number(libraryData.lib.libInfo.latitude)" :lng="Number(libraryData.lib.libInfo.longitude)" :draggable="true">
        ...
        >
          <template v-slot:infoWindow><div style="padding: 10px">{{ libraryData.lib.libInfo.libName }}</div></template>
        </KakaoMapMarker>
      </KakaoMap>
    </div>
    <div v-else>
...
.tab {
  ...
  font-weight: 500;
  position: relative; /* 자식 요소(.tab-active::before)의 위치 기준 */
}
